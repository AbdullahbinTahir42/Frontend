...
export default function AdminPanel() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-yellow-100 to-yellow-200 p-6">
      {notification && (
        <div
          className={`fixed top-4 right-4 p-4 rounded shadow z-50 ${
  ...
  <div className="bg-white rounded-lg shadow border border-gray-200 mb-8">
    ...
    <div className="divide-y divide-gray-200">
      {verifyingProfiles.map((profile) => (
        <div key={profile.id} className="p-4 hover:bg-gray-50 flex flex-col sm:flex-row justify-between items-center sm:items-start">
          <div className="flex items-center space-x-4 sm:flex-row flex-col sm:items-start">
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      {app.resume_filename && (
                        <div className="mt-2">
                          <a
                            href={`http://api.hr.growvy.online/resumes/${app.resume_filename}`}
                            target="_blank"
      ...
      >
        ...
        <div className="mt-4 sm:mt-0">
          {u.payment_status === "Pending" && (
            <div className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded text-xs mb-2 sm:mb-0">
              Receipt Pending
            </div>
          )}
          {u.payment_status === "Verifying" && (
            <button
              onClick={() => handleMarkPayment(u.id)}
              className="bg-[#ea9f6f] text-white px-3 py-1 rounded text-xs hover:bg-orange-500 mb-2 sm:mb-0"
            >
              Verify Payment
            </button>
          )}
          ...
          <div className="p-4 space-y-4">
            {jobs.length === 0 ? (
              ...
              jobs.map((job) => (
                <div
                  key={job.id}
                  className="border-b border-gray-200 pb-4 hover:bg-gray-50 p-2 rounded"
                >
